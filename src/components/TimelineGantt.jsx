...
export default function TimelineGantt({
  ...
  const normalizedTasks = useMemo(() => {
    return (tasks || [])
      .map((task) => {
        ...
        const safeEnd = end.isBefore(start)
          ? start.endOf("day")
          : end;
  ...
  // Build timeline range using Math.min / Math.max
  const { min, max } = useMemo(() => {
    ...
    const minTime = Math.min(
      ...normalizedTasks.map((t) => +t._start)
    );
    const maxTime = Math.max(
      ...normalizedTasks.map((t) => +t._end)
    );
  ...
  const percentageFromRange = (d) => {
    const clamped = Math.min(
      Math.max(0, d.diff(min, "millisecond")),
      totalMs
    );
      ...
      <div className="relative min-w-0">
        {normalizedTasks.map((task, index) => {
          ...
          const widthPercent = Math.max(
            endPercent - leftPercent,
            0.3 // ensures visibility for short tasks
          );
